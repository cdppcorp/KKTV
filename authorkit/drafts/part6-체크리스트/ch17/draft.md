...
보안 체크 플로우차트는 `diagram.md`를 참고하십시오.
...
- [ ] 로그인 실패 시 "아이디가 틀렸습니다" 또는 "비밀번호가 틀렸습니다"처럼 구체적인 정보를 노출하지 않습니까? (올바른 방식: "아이디 또는 비밀번호가 일치하지 않습니다")
...
- [ ] 이메일, 전화번호 등의 입력 형식에 대한 정규식(Regex) 검증이 적용되어 있습니까?
...
> **💡 팁:** Vercel, Netlify, Railway 같은 플랫폼을 사용하면 HTTPS 설정이 자동으로 적용됩니다. 그러나 보안 헤더와 CORS 설정은 여전히 여러분이 직접 구성해야 합니다.
...
AI 도구는 코드를 생성하는 것뿐만 아니라 코드를 검토하는 데도 활용할 수 있습니다. 이 절에서는 Claude, ChatGPT, Cursor 등의 AI 도구에 붙여넣기하여 바로 사용할 수 있는 보안 검토 프롬프트 템플릿을 제공합니다.
...
3. 브루트포스 공격(Brute Force Attack)에 대한 방어가 있는가?
4. 비밀번호 재설정 흐름이 안전한가?
...
2. 사용자가 다른 사용자의 데이터에 접근할 수 있는 IDOR 취약점이 있는가?
3. 입력값 검증이 누락된 곳이 있는가?
4. 응답에 불필요한 데이터가 포함되어 있는가?
...
7. 불필요한 파일(.env, .git, debug 엔드포인트)이 외부에 노출되지 않는가?
...
> **💡 팁:** 프롬프트 6은 코드 생성과 보안 검토를 동시에 요청하는 방식입니다. AI에게 자체 검토를 요청하면 일반적인 코드 생성보다 보안 품질이 향상됩니다. 이 프롬프트를 프로젝트의 시스템 프롬프트(System Prompt) 또는 `.cursorrules` 파일에 포함시키면 매번 작성하지 않아도 됩니다.
...
이 절에서는 바이브 코딩 환경에서 가장 빈번하게 발생하는 보안 실수 10가지를 순위별로 정리하였습니다. 각 항목에 대해 실수의 내용, AI 도구에서 이 실수가 발생하는 이유, 그리고 수정 방법을 설명합니다.
...
**왜 AI에서 발생하는가:** AI의 학습 데이터인 튜토리얼과 예제 코드 대부분이 설명의 편의를 위해 비밀키를 하드코딩합니다. AI는 이 패턴을 "일반적인 코딩 방식"으로 학습합니다. 또한 AI는 여러분의 배포 환경을 알 수 없으므로 환경 변수 설정을 가정할 수 없습니다.
...
**무엇인가:** 사용자 입력을 문자열 결합(String Concatenation)이나 f-string으로 SQL 쿼리에 직접 삽입하는 것입니다.

**왜 AI에서 발생하는가:** 문자열 포매팅을 사용한 SQL 쿼리가 코드가 짧고 직관적이기 때문에 AI가 이 방식을 선호합니다. 특히 간단한 쿼리를 요청하면 AI는 ORM이나 매개변수화된 쿼리 대신 직접 SQL 문자열을 구성하는 경향이 강합니다.
...
**왜 AI에서 발생하는가:** "회원가입 기능 만들어줘"라는 요청에 AI는 핵심 로직(사용자 생성, 데이터 저장)에 집중하고, 비밀번호 해싱은 부가적인 기능으로 취급하여 생략하는 경우가 많습니다. 특히 빠른 프로토타입 코드에서 이 문제가 두드러집니다.
...
**무엇인가:** 상태를 변경하는 POST 요청(회원정보 수정, 게시글 작성, 비밀번호 변경 등)에 CSRF(Cross-Site Request Forgery) 토큰이 없는 것입니다.

**왜 AI에서 발생하는가:** AI는 HTML 폼의 기본 구조(input, button, action)에 집중하며, CSRF 토큰은 보안 계층이므로 명시적으로 요청하지 않으면 생략합니다. 또한 프레임워크의 CSRF 미들웨어 설정은 별도 파일에서 이루어지므로 AI가 단일 파일 컨텍스트에서 놓치기 쉽습니다.

**수정 방법:** 프레임워크의 CSRF 보호 기능을 반드시 활성화하고, 모든 폼에 CSRF 토큰 필드를 추가하십시오. Flask에서는 `flask-wtf`의 `CSRFProtect`, Django에서는 `{% csrf_token %}` 템플릿 태그를 사용합니다.

---

### 5위: 디버그 모드 활성화 상태로 배포

**무엇인가:** Flask의 `debug=True`, Django의 `DEBUG=True` 등 개발용 디버그 설정이 프로덕션 환경에서도 활성화된 상태인 것입니다.

**왜 AI에서 발생하는가:** AI가 생성하는 서버 실행 코드는 거의 100% 디버그 모드가 활성화되어 있습니다. 이는 개발 중 편의를 위한 것이지만, AI는 "이 코드가 프로덕션에 배포될 것"이라는 맥락을 고려하지 않습니다.
...
**왜 AI에서 발생하는가:** AI에게 "폼 유효성 검사를 추가해줘"라고 요청하면 눈에 보이는 프론트엔드 검증(HTML5 required 속성, JavaScript 검증)을 먼저 구현합니다. 서버 측 검증은 별도로 요청하지 않으면 누락되기 쉽습니다.

**수정 방법:** 모든 입력값은 반드시 서버 측에서 다시 검증해야 합니다. 클라이언트 검증은 사용자 편의를 위한 것이며, 보안을 위한 것이 아닙니다. 공격자는 브라우저의 개발자 도구(DevTools)를 사용하여 클라이언트 검증을 쉽게 우회할 수 있습니다.
...
**왜 AI에서 발생하는가:** AI는 개발 단계를 가정하고 코드를 생성하므로, 에러를 상세하게 출력하는 것을 "도움이 되는 기능"으로 취급합니다. 또한 AI는 전체 에러 처리 전략보다 개별 기능 구현에 집중하므로, 포괄적인 에러 핸들러를 구성하지 않는 경우가 많습니다.
...
**무엇인가:** CORS(Cross-Origin Resource Sharing) 설정에서 모든 출처(Origin)를 허용하는 `*` 와일드카드를 사용하는 것입니다.

**왜 AI에서 발생하는가:** 개발 중 CORS 에러는 매우 흔하며, AI는 이 에러를 가장 빠르게 해결하기 위해 `Access-Control-Allow-Origin: *`을 설정합니다. 로컬 개발에서는 문제가 없지만 프로덕션에서는 다른 도메인에서 여러분의 API에 자유롭게 접근할 수 있게 됩니다.
...
**무엇인가:** 사용자가 업로드하는 파일에 대해 확장자, 크기, MIME 타입 등을 검증하지 않거나, 업로드된 파일을 웹 서버가 직접 실행할 수 있는 경로에 저장하는 것입니다.

**왜 AI에서 발생하는가:** AI에게 "파일 업로드 기능 만들어줘"라고 요청하면 파일을 받아서 저장하는 기본 기능만 구현합니다. 파일 유형 검증, 크기 제한, 안전한 저장 경로 설정 등의 보안 로직은 명시적으로 요청하지 않으면 누락됩니다.
