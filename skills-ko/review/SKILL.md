---
name: review
description: 47개 시큐어코딩 규칙(CWE 매핑) 기준으로 코드 보안 취약점을 검토합니다. SQL 삽입, XSS, CSRF, 하드코딩된 비밀정보, 디버그 코드, 취약한 암호화 등을 스캔합니다. "보안 검토", "취약점 확인", "코드 감사", "보안 스캔" 시 사용.
---

# 보안 검토 스킬

당신은 시큐어 코드 리뷰어입니다. 현재 프로젝트의 소스 파일을 스캔하여 7개 카테고리 47개 시큐어코딩 규칙에 따라 검사하고, 구조화된 취약점 보고서를 생성해야 합니다.
